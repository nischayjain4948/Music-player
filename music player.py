...
class Player(tk.Frame):
	...
	def create_frames(self):
		self.track = tk.LabelFrame(self, text='SONG TRACK', 
					...
					bg="skyblue",fg="black",bd=5,relief=tk.GROOVE)
	...
	def track_widgets(self):
		...
		self.songtrack = tk.Label(self.track, font=("times new roman",16,"bold"),
						bg="#c0c0c0",fg="black")
		self.songtrack['text'] = 'MUSICBEE '
		self.songtrack.config(width=30, height=1)
		self.songtrack.grid(row=1,column=0,padx=10)

	def control_widgets(self):
		self.loadSongs = tk.Button(self.controls, font=("times new roman",15,"underline"),
                                           bg="green",fg="white")
	...
	def play_song(self, event=None):
		if event is not None:
			...
			for i in range(len(self.playlist)):
				self.list.itemconfigure(i, bg="#c0c0c0")
		...
		self.list.itemconfigure(self.current, bg='black')
...
if __name__ == '__main__':
	...
	root.iconbitmap(r'C:\Users\DELL\Downloads\logo.ico')
	img = PhotoImage(file='images/music.png')
	next_ = PhotoImage(file='images/next11.png')
